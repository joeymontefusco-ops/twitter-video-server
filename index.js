...
${sectionDescriptions}`;
  ...
  for (const section of sections) {
    console.log(`[gemini] Validating section ${section.number}: "${section.title}"...`);

    // Use Gemini estimated timestamp if available, otherwise spread evenly
    const estimated = estimatedTimestamps.find(t => t.number === section.number);
    const estimatedStart = estimated ? estimated.timestamp_sec : 0;

    console.log(`[gemini] Section ${section.number} estimated at ${estimatedStart}s`);

    // Scan window centered around estimated timestamp
    const windowStart = Math.max(0, estimatedStart - 5);
    const windowEnd = estimatedStart + 20;
    ...
    for (let t = windowStart; t <= windowEnd; t += FRAME_INTERVAL) {
      ...
      try {
        ...
        let res;
        const imageModels = ['gemini-2.5-flash', 'gemini-2.0-flash-001', 'gemini-2.0-flash', 'gemini-2.5-flash-lite'];
...
Does this frame show ANY of the following:
1. Colored lines (yellow, red, pink, blue, purple) drawn on the field grass extending from players showing receiver routes
2. A SHOW PLAY panel open on the right side of the screen
3. Player route icons (△ ○ □ X) floating above receivers WITH at least one colored line visible anywhere on the field

Answer "no" for:
- Players standing with ONLY icons above their heads and ZERO colored lines on the field grass
- Post-snap action where ball is already thrown
- Formation selector or playbook menus
...
Answer only "yes" or "no":`,
            ...
            console.error(`[gemini] Image model ${model} failed:`, modelErr.message);
          }
        }

        if (!res) continue;

        const answer = res.data?.candidates?.[0]?.content?.parts?.[0]?.text?.trim().toLowerCase() || 'no';
    ...
    if (foundTimestamp === null) {
      console.log(`[gemini] Section ${section.number}: nothing in window, expanding ±30s...`);
      const expandStart = Math.max(0, estimatedStart - 20);
      const expandEnd = estimatedStart + 40;
...
Answer only "yes" or "no":`,
    ...
    // Use found timestamp or fall back to estimated
    const finalTimestamp = foundTimestamp !== null ? foundTimestamp : estimatedStart;
    results.push({ number: section.number, timestamp_sec: finalTimestamp });
    console.log(`[gemini] Section ${section.number} final timestamp: ${finalTimestamp}s`);
  }
